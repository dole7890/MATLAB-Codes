function [out_profile,out_errors,out_IMU_bias_est,out_clock,out_KF_SD,out_gnss,out_kf] =...
    ...
    IMU_errors,GNSS_config,TC_KF_config,mode,acc,gyr,truth,dat_rinex,eph,settings)
...
GNSS_biases = zeros(size(GNSS_biases));
%}
if strcmp(mode,'sim')
    ...
    % DF (data filter)
    idx_bad = find(dat_rinex.data(idx,4) == 0);
    idx(idx_bad) = [];
    ...
    relsvs = [];
    for ii = 1:length(idx)
        [~, x, bsv, relsv, xdot] = broadcast_eph2pos_etc(eph,[dat_rinex.data(1,1) ts_rinex(1)],dat_rinex.data(idx(ii),3));
        xs(ii,:)=x;
        xdots(ii,:)=xdot;
        bsvs(ii,:) = bsv;
        relsvs(ii,:) = relsv;
    ...
    GNSS_measurements = [dat_rinex.data(idx,4) -dat_rinex.data(idx,6)/1575.42e6*settings.c xs xdots];
    no_GNSS_meas = length(GNSS_measurements(:,1));
    S = dat_rinex.data(idx,7);
    
%     for ii=1:length(no_GNSS_meas)
%         if (dat_rinex.data(idx(ii),4) == 0) || (dat_rinex.data(idx(ii),7) == 0) || ~settings.dualfreq
%             continue
%         else
%             [PRIF,~,~] = ionocorr(dat_rinex.data(idx(ii),4),settings.f1,dat_rinex.data(idx(ii),7),settings.f2);
...
imu = zeros(length(truth(:,1)),7);
imu(:,1) = truth(:,7);
ts_imu = imu(:,1);
% acc_idx = find(acc(:,1)<ts_imu(1));
% gyr_idx = find(gyr(:,1)<ts_imu(1));
for ii=2:length(ts_imu)
    acc_idx = intersect( find(acc(:,1)<ts_imu(ii)),find(acc(:,1)>ts_imu(ii-1)) );
    gyr_idx = intersect( find(gyr(:,1)<ts_imu(ii)),find(gyr(:,1)>ts_imu(ii-1)) );
    imu(ii,2:4) = mean(acc(acc_idx,2:4));
    imu(ii,5:7) = mean(gyr(gyr_idx,2:4));
end
...
%         time = TOW(epoch);
        time_imu = truth(epoch,7);
    end

    % Time interval
    tor_i = (time_imu - old_time)/1000;
...
%         old_true_C_b_e,true_v_eb_e,old_true_v_eb_e,old_true_r_eb_e);
    ...
    if strcmp(mode,'sim')
        ...
        meas_omega_ib_b = deg2rad(imu(epoch,5:7)'); % novatel uses deg
        
%         rot = [0 1 0;1 0 0;0 0 -1]; % novatel
        rot = [0 0 1;-1 0 0;0 -1 0]; % novatel
...
%         GNSS_config.epoch_interval = 0.2;
    ...
    if 1%(time_imu - time_last_GNSS)+0.001 >= GNSS_config.epoch_interval
%     if time_GNSS > time_imu

        
%         plot(rad2deg(true_lambda_b),rad2deg(true_L_b),'r.')
        
        GNSS_epoch = GNSS_epoch + 1;
%         tor_s = time_imu - time_last_GNSS;  % KF time interval
        tor_s = tor_i;
%         time_last_GNSS = time_imu;
...
%             GNSS_measurements,no_GNSS_meas,GNSS_config.init_est_r_ea_e,[0;0;0],bsvs,relsvs,settings,satPosLast,S);
            [gnss_r,gnss_v,~,tropo] = GNSS_LS_position_velocity_ION(...
            GNSS_measurements,no_GNSS_meas,[0;0;0],[0;0;0],bsvs,relsvs,settings,satPosLast,W);
            out_gnss(epoch,:) = gnss_r';
            GNSS_measurements(:,1) = GNSS_measurements(:,1) - tropo;
            [gnss_r,gnss_v]
            end
            %}
        else
            idx = find(dat_rinex.data(:,2)==ts_rinex(epoch));
            ...
            S = dat_rinex.data(idx,7);
            cno = sort(S);
%             idx_bad = find(dat_rinex.data(idx,7)<cno(4));
%             idx(idx_bad) = [];
            ...
            relsvs = [];
            for ii = 1:length(idx)
                [~, x, bsv, relsv, xdot] = broadcast_eph2pos_etc(eph,[dat_rinex.data(1,1) ts_rinex(epoch)],dat_rinex.data(idx(ii),3));
                xs(ii,:)=x;
                xdots(ii,:)=xdot;
                bsvs(ii,:) = bsv;
                relsvs(ii,:) = relsv;
...
%             for ii=1:length(no_GNSS_meas)
%                 if (settings.PRIF~=1) || (dat_rinex.data(idx(ii),4) == 0) || (dat_rinex.data(idx(ii),7) == 0) || ~settings.dualfreq
%                     1
%                 else
%                     [PRIF,~,~] = ionocorr(dat_rinex.data(idx(ii),4),settings.f1,dat_rinex.data(idx(ii),7),settings.f2);
%                     GNSS_measurements(ii,1) = PRIF;
...
%             GNSS_measurements,no_GNSS_meas,GNSS_config.init_est_r_ea_e,[0;0;0],bsvs,relsvs,settings,satPosLast,S);
        ...
        [est_C_b_e,est_v_eb_e,est_r_eb_e,est_IMU_bias,est_clock,P_matrix] =...
            TC_KF_Epoch_ION(GNSS_measurements,no_GNSS_meas,tor_s,est_C_b_e,...
...
%         v_pred_err(epoch) = norm(est_v_eb_e - v_old);
    ...
    % Convert navigation solution to NEDw
...
%     old_true_C_b_e = true_C_b_e;
    ...
    plot(lla(2),lla(1),'b.')
    plot(in_profile(epoch,3),in_profile(epoch,2),'r.')
    ...
    err_gps(epoch) = norm(out_gnss(end,:) - lla2ecef([in_profile(epoch,2),in_profile(epoch,3),in_profile(epoch,4)]));
    err_kf(epoch) = norm(old_est_r_eb_e' - lla2ecef([in_profile(epoch,2),in_profile(epoch,3),in_profile(epoch,4)]));
