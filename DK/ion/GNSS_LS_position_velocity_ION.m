function [est_r_ea_e,est_v_ea_e,est_clock,tropo] = GNSS_LS_position_velocity_ION(...
    GNSS_measurements,no_GNSS_meas,predicted_r_ea_e,predicted_v_ea_e,bsvs,relsvs,settings,satPosLast,W)
...
iter = 0;
while (test_convergence>1e-7 && iter < 100) || iter<5
    iter = iter+1;
    ...
    if iter == 1
        ...
        tropo = tropomodel(EL, settings.zd);
%         tropo = zeros(no_GNSS_meas,1);
    end
