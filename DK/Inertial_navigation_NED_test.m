...
in_profile = zeros(length(imu),10);
in_profile(:,1) = imu(:,1);
in_profile(:,2) = ones(length(imu(:,1)),1)*deg2rad(39.7392);
in_profile(:,3) = ones(length(imu(:,1)),1)*deg2rad(-104.9903);
in_profile(:,4) = ones(length(imu(:,1)),1)*1600;
...
plot(out_profile(:,7))
ts = 100;
plot((out_profile(1:ts,5).^2+out_profile(1:ts,6).^2+out_profile(1:ts,7).^2).^0.5)
xyz1 = lla2ecef([rad2deg(out_profile(ts,2)) rad2deg(out_profile(ts,3)) out_profile(ts,4)]);
xyz2 = lla2ecef([rad2deg(in_profile(ts,2)) rad2deg(in_profile(ts,3)) in_profile(ts,4)]);
norm(xyz1-xyz2)
